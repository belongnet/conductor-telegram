...
Discovery matches canonical repository remotes to Conductor projects and creates one topic per native workspace, including sleeping workspaces. It preserves existing sessions and models, sends the working or most recently updated session's latest visible reply once as context, and forwards new replies from every session. It does not launch, wake, stop, or replay existing work. That context snapshot does not select a Telegram default. With multiple visible threads, the first ambiguous message and its prepared attachments wait for an explicit choice from a picker showing thread names and models. The choice becomes the Telegram default; `/threads` changes it, and a single visible thread needs no choice. Telegram cannot read the tab open in the Conductor app. Unknown model settings or ambiguous repository mappings require reconciliation.

Replying to a forwarded message targets that exact session, including after a gateway restart. Queued messages and media retain their chosen session through preparation, retries, and later selection changes. The send receipt names the actual thread and model and includes its Conductor link; a restart can restore that receipt without resending the task. Once a message is delivered, replies to the original Telegram message retain its destination too, including messages that started a thread with `/threads new <prompt>`.

Text and voice replies use the same targeting. Voice is downloaded privately, converted with FFmpeg and transcribed with Whisper in the separate media worker; Conductor receives the resulting text. A failed transcription never submits an empty task. A photo album is handled as one message: Telegram delivers it as one update per file, and the gateway sends those files as a single task, wherever the caption sits. A file that arrives after the album was handled joins that same task for ten minutes, provided the task reached Conductor; a file it cannot join is answered with a note to send it again on its own. Telegram hands a bot at most 20 MB of any one file; a larger one is refused with that reason rather than retried. Topic service events do not become prompts, and commands addressed to other bots are ignored. When Conductor archives or deletes a workspace, its topic closes and its history remains. Existing repository topics are retained. During migration, `TELEGRAM_CLOUD_SYNC_INPUT=commands` permits only commands explicitly addressed to this bot in the sync group, plus its own button callbacks. Set it to `all` (the default) after disabling the previous gateway's consumer to enable ordinary text and voice replies. This setting does not stop another bot from processing messages; do not send plain text or voice into a shared group while its old consumer remains active.
...
Use `/projects` or `/repos`, `/run <project ID> <task>`, `/send`, topic messages, or replies to forwarded messages. `/threads new <prompt>` creates a native session; `/threads` selects one. A new workspace's topic opens under the first line of its task; as soon as Conductor has titled the workspace's first thread, the workspace takes that title in Conductor and in its topic, keeping any tag another tool added. Only a topic the gateway opened for the workspace, and that still follows it, is renamed. Titles are applied a few seconds apart, and a failed rename is retried with backoff. Threads the gateway opens are named for their task, `Review PR #N`, or `Recovery`. `/fleet` uses the native read-only transcript view. `/lanes` retains the existing HTTP controls and human approval checks; its independently supervised worker owns orchestration, ordering, capacity, and merge gates.
...
The AI router uses a dedicated native workspace/session, serialized requests, validated target IDs, and owner confirmation before work begins. Configure its project with `TELEGRAM_CLOUD_ROUTER_PROJECT_ID`. At migration time, repository topics must map to a verified canonical Git remote and project ID; a historical Mac basename never authorizes a destination there. At runtime the gateway resolves an unmapped topic from its own repository name against the project catalog, and asks whenever that is not a single match; see the migration section below.

`TELEGRAM_CLOUD_REVIEW_POLICY=native` enables `/review [PR number or URL]`. Bare `/review` finds an open PR from a still-open cached URL, the transcript, or the branch; `/review 500` or a GitHub PR URL selects one explicitly. Several open PRs get buttons on the status card instead of a guess; when none are found, buttons offer to ask the agent to self-review or `/ship`. A choice retains the selected thread, a PR selection preserves any review instructions, and only one choice on each card can start work. With native reviews disabled, `/review` offers to ask the agent to review its own diff. A GitHub token that cannot read the repository says so. Reviews use a separate session and a provider different from the task author. Existing `TELEGRAM_REVIEW_AGENT_TYPE` and `TELEGRAM_REVIEW_MODEL` settings are honored; an unavailable or same-author reviewer is rejected before execution. They receive findings-only instructions with normal Conductor permissions. They are bound to the exact head and base commits; subsequent changes invalidate completion. No completion notice creates a merge intent, approval, or authorization to bypass checks. The independent lanes worker retains its existing merge checks.

Recovery reconciles transcript, workspace lifecycle, and session status. A terminal disconnection first gets one continuation in the same session. Provider exhaustion (including depleted usage credits), unavailable or inaccessible models, and repeated disconnects advance through the configured primary, then the exact model ladder Claude `fable-5-1` → Codex `gpt-6-astra` → Claude `opus-5-1m` → Codex `gpt-6-sol` → Cursor `grok-4.7`, omitting exact attempted agent/model routes. The fallback notice names the model that stopped, why, and the model continuing, for example `claude (fable-5-1) ran out of usage credits. Continuing in codex (gpt-6-astra); existing work is kept.` Replies to it follow the replacement, never the thread that stopped. A task's replacement thread is named `Recovery` and a review's keeps `Review PR #N`; in a topic with more than one thread, each forwarded reply is labelled with its thread and model, such as `Recovery · gpt-6-astra`, and a recovered task's completion names the model that finished. `TELEGRAM_CLOUD_PROVIDERS` must list only configured providers and include the primary. Each task/review has a separate persisted recovery episode. A replacement rechecks that its predecessor is still failed before it starts. An explicit Telegram thread selection transfers to the replacement only if recovery replaces that selected session. Recovering a different thread makes the next ambiguous topic message ask for a choice again. Uncertain workspace/session creation is reconciled by its recorded identity and never blindly repeated. User-stopped tasks remain stopped; start a new `/run` to resume deliberately.
...
3. Voice notes transcribe; photo/document captions reach the agent; a captioned photo album arrives as one task holding every photo; an agent-created file comes back to Telegram. Refresh an expired link. Another workspace's credential must receive no file or decision data.
4. Bare `/review` in a topic with a PR names it on the card. `/review 500` works. A topic without a PR shows buttons within seconds. Change the head commit and verify previous completion becomes invalid. Review completion cannot grant merge approval.
...
The identity file has `repositories: [{path, remote, verifiedBy: "git-remote"}]` from actual source `git remote get-url origin` reads, and `projects: [{id, name, gitRemote}]` from the authenticated Conductor API. At migration time, only unique full-remote matches map repository topics. The report lists unresolved topics, native bindings, and creation intents. An unresolved topic is not a dead end. The gateway matches the topic's repository name against the project catalog, by project name or by the repository name in a project's remote, and records a single match so the topic routes itself from then on; the acknowledgement names the project it linked. No match or more than one match answers with a ranked picker and states that the message was not sent, so nothing reaches Conductor on a guess. A confirmed tap or `/link <project ID>` sets the mapping directly, and answering an offer retires its buttons so a tap from scrollback cannot re-point the topic later. `/run` and `/cloud` name a project for one task: they adopt an unlinked topic and say so, and never re-point a topic that is already linked. A repo topic holds one workspace at a time: the first task adopts the topic and later messages continue that workspace, while `/run` rolls the topic onto new work. A workspace living there never renames or closes the topic. A link whose project a catalog read stops listing is kept rather than dropped: the gateway asks again with the current project marked instead of launching against a project it cannot currently see. One burst of messages is answered once; a later attempt is always answered. Preserve unresolved historical workspaces; never guess a cloud destination. Existing native message IDs, cursors, queued updates, decisions, and pending operations remain in the database. Verified legacy outboxes retain original message IDs/text during adoption. Unresolved creation intents block replay and need reconciliation.
