...
| `/lanes` | `/lanes [pause\|resume\|hold\|release\|validate\|retry\|provider-disable\|archive-approval\|shadow\|cutover\|rollback]` | Durable lane status and audited controls when Manifest v2 is configured; legacy scheduler controls otherwise. Retirement evidence is accepted only by the local CLI. |
...
| `/threads` | `/threads [workspace]` | List Conductor threads, choose the thread for Telegram replies, or start a new thread |
...
2. **Send inside the workspace's forum topic** — skill shortcuts and `/skill` / `/gstack` pick up the topic's workspace automatically. Plain messages use the topic's Telegram thread selection. In a synced Cloud workspace with multiple threads and no explicit selection, the message waits for a thread choice.
...
Conductor 0.72+ threads are mirrored into the same Telegram workspace topic. When a workspace has multiple visible Conductor sessions, forwarded messages include a `🧵` thread label. Use `/threads` in the topic to choose the thread for Telegram replies or `/threads new <prompt>` to start a new one. In cloud-only mode, the picker shows each thread's model, and so does the label on each forwarded reply, such as `Recovery · gpt-6-astra`; Telegram's selection is separate from the tab open in the Conductor app. A workspace started from Telegram in cloud-only mode opens its topic under the task's first line, then takes the title Conductor gives its first thread, in Conductor and in the topic. `/rename` overrides it.
...
For synced workspaces, the first ambiguous topic message is saved with its attachments until you choose a thread. That choice becomes the Telegram default; a workspace with one visible thread sends directly. Queued messages and media keep their chosen session through preparation, retries, and later `/threads` changes. The send receipt names the actual thread and model and includes a Conductor link. Once delivered, replies to your original Telegram message, including `/threads new <prompt>`, return to that same thread.
...
Copy [docs/lanes.manifest-v2.example.json](docs/lanes.manifest-v2.example.json) to `~/.conductor-telegram/lanes.manifest.v2.json`. Manifest v2 accepts only the approved provider/model/cap map (`claude/fable-5-1` primary or `claude/opus-5-1m` backup at 3, `codex/gpt-6-astra` primary or `codex/gpt-6-sol` backup at 2, `cursor/grok-4.7` at 2), rejects Sonnet, legacy Sol/Grok identifiers, cross-provider models, runtime IDs, and dependency cycles, and verifies every prompt SHA-256 both on startup and immediately before a commissioned delivery. Recurring schedules are `daily`, `weekly`, or `every <positive integer><m|h|d>`. Recurring lanes are inert unless `enabled` is explicitly `true`, and they may never auto-merge.
...
conductor-telegram lanes hold <lane> --reason "operator reason"
conductor-telegram lanes release <lane>
conductor-telegram lanes validate <lane>
conductor-telegram lanes retire <lane> --merged-sha <full-sha> --evidence-json /path/to/evidence.json
...
Lane controls are scoped to the exact active manifest revision. `hold` atomically records the precise run status/stage and pauses only that lane; a queued `release` is human-approved, binds the exact current hold control, and restores that exact state only if the hold has not been replaced. `validate` is a human-approved, one-run permit bound to the exact current hold, run, and merged SHA for an already merged, held one-shot lane while the controller remains globally paused; rejected merged-CI or deterministic-validation evidence atomically fails its active attempt, releases capacity, and returns it to a durable hold without opening repair work. `retire` is human-approved, accepts only a regular bounded evidence file whose documents exactly name the active revision/lane/latest run/merged SHA, and atomically records immutable legacy-adoption evidence before ending the run as `validated`. A retirement packet must contain both a `merge_record` whose payload has `merged:true` and at least one successful validation receipt: fully green `required_checks` with empty missing/non-passing lists, `deterministic_validation` with `passed:true`, or `canonical_replay` with both `verified:true` and `passed:true` plus a non-empty verdict or receipt summary. Merge-only, failed, and empty packets are rejected. Direct recurring dependents must already be held or terminal. Retirement is intentionally unavailable through Telegram so a chat message cannot act as a large privileged evidence packet.
...
Delivery requires an adversarial reviewer distinct from the author, then two current-head final attestations from distinct providers. Attestations are accepted only from commissioned attempts with the exact nonce, run, stage, provider, head SHA, tag, and body hash, and terminal PR/review/validation output is not consumed until its Conductor session is idle. Merge is bound to owner/repository/base/head branch/head SHA and requires the configured case-sensitive named checks to pass; unrelated failing checks are ignored only when an explicit allowlist is configured, and a missing required check remains unavailable rather than becoming a repair signal. An empty allowlist preserves the Git host's aggregate gate. Both Git hosts reassert the expected head after a merge response. Completion requires the same exact allowlist on CI for the stored merged SHA and the manifest's exact command/probe evidence; GitLab legacy evidence must use canonical GitLab repository/MR identity and can never be repaired through a guessed SHA mapping. Commentary markers alone do not count. Terminal workspace grace starts at the original terminal transition, and an untagged archive approval is bound to an immutable, expiring list containing the current workspace ID. Archive hygiene performs a second live-session preflight after recording its intent and immediately before mutation; a newly working session pauses the controller instead of being archived. The controller observes repository-native CI but never deploys, publishes, performs outreach, spends money, or changes secrets.
