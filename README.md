...
Each tick (every `intervalMinutes`, and on `/lanes run`) looks up workspaces named `[lane:<id>:…` with a per-lane name filter, or an explicit `sessionId` / `workspaceId`. A listing outage skips the tick instead of creating a duplicate workspace. A lane is **working** when its session status is `working`, **done** when the last idle-turn assistant text contains a GitHub pull-request URL (tool payloads are ignored), **initializing** when no user message has been recorded yet, **paused** when it exists and is idle but not done, **unknown** when status cannot be read, and **not created** otherwise. Providers under `maxActive` nudge the first paused, dependency-ready lane whose last user message is older than `gapHours` (until `maxNudges`), else re-send the configured prompt to an initializing lane, else create the first not-created lane for that provider (or `"any"`). Failed creates and nudges are logged and the tick moves on.
